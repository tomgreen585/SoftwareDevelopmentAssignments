...
public class Hand implements Cloneable, Iterable<Card> {
	...
	@Override
	public Hand clone() {
		Hand clonedHand = new Hand(); // initialize Hand object
		for (Card card : cards) { // Iterate
		    if (card != null) {clonedHand.cards.add(card);} // Add card to cloned object   
		}
		return clonedHand; // Return shallow copy
	}
